...
def compute_top_label_confidence_and_correctness(entries):
    ...
    # eval_set = "forget_mcqa_prob" 
    eval_set = "retain_mcqa_prob" 

    entries = entries[eval_set]
    ...
    for _, e in entries['value_by_index'].items():
        ...
        pred_lbl = e.get("generated choice")
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="calibration_logs/plots")
    args = parser.parse_args()
    
    if os.path.exists(args.outdir):
        print(f"Output directory {args.outdir} already exists. Using input filename {os.path.basename(args.input).replace('.json','')}.")
        args.outdir = f"calibration_logs/plots/{os.path.basename(args.input).replace('.json','')}"
...
# COMMANDS
'''
python plot_reliability.py --input saves/eval/relu_phi-1_5_retain90_batch1_debug/RELU_EVAL.json --bins 10 --outdir saves/eval/plots
'''